    ...
    function GameViewModel() {
       ...
       // Click handler for back button
        ...
        self.makeGame = function() {
            ...
            var startCardModel = self.activeStartCard();
            startCardModel.card.then(function(startCard) {
                // Simulate a deal
                var startCardContext = startCard.getContext && startCard.getContext(undefined, game.inventory());
                var startCardDeal = (startCard.deal && startCard.deal(undefined, startCardContext));
                var startCardInventory = { id: self.activeStartCard().id() };
                var startCardParams = startCardDeal && startCardDeal.params;
                if (startCardParams && _.isObject(startCardParams))
                    _.extend(startCardInventory, startCardParams);
                startCard.keep && startCard.keep(startCardDeal, startCardContext);
                startCard.releaseContext && startCard.releaseContext(startCardContext);

                game.inventory().cards.push(startCardInventory);
                ...
                _.forEach(teamInfo, function (info) {
                    ...
                    _.forEach(info.workers, function (worker) {
                        var starPct = (worker.star.distance() / maxDist);
